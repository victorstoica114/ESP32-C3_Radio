...
The bridge forwards AT command lines unchanged and does not change the CC1352P
radio state at startup. Default RX behavior belongs in the CC1352P modem
firmware. Non-AT text lines are wrapped as `AT+SEND=<text>` before they are sent
to the CC1352P modem, so a serial terminal can transmit radio payloads by typing
plain text such as `hello`.

For transparent use, modem `OK` replies generated by bridge-side payload sends
are hidden, and incoming modem `+RX:...` / `+RXHEX:...` records are printed as
payload only, matching the SX127x AT-command sketches' default behavior.

The default target UART baud toward the CC1352P is `1000000`, matching the E79
AT modem firmware.
...
```powershell
pio run -e e79-esp32-bridge -t upload --upload-port COM19
```
